        ...
        try {
            ...
            if ($search) {
                $query->where(function ($q) use ($search) {
                    $q->where('assignment_status.name', 'ilike', '%' . $search . '%');
                });
            }

            $sortField = $with_sort == 1 ? 'assignment_status.name' : 'assignment_status.created_at';
            $sortDirection = $with_sort == 1 ? 'asc' : 'desc';

            $data = $query
                ->orderBy($sortField, $sortDirection)
                ->paginate($perPage)
                ->withPath(env('DASHBOARD_URL') . '/assignment-status')
